...
public:
    ...
    void offerPokemonChoices(Player& p) {
        std::cout << Utils::getPrintName(name) << "You must be eager to start your adventure. But first, you’ll need a Pokemon of your own!";
        Utils::waitForEnter();
        ...
        switch ((PokemonChoice)choice) {
            ...
            default:
                ...
                std::cout << Utils::getPrintName(name) << "Well, lets see, Ahhh.... here it is, take Pikachu, an electric Pokemon" << std::endl;
                break;
        }
    }
    ...
    void explainMainQuest(const Player& p) {
        std::cin.ignore();
        std::cout << Utils::getPrintName(name)   << "Oak-ay " << p.name <<", I am about to explain you about your upcoming grand adventure.";
        Utils::waitForEnter();
        std::cout << Utils::getPrintName(name)   << "You see, becoming a Pokémon Master is no easy feat. It takes courage, wisdom, and a bit of luck.";
        Utils::waitForEnter();
        std::cout << Utils::getPrintName(name)   << "Your mission, should you choose to accept it (and trust me, you really don’t have a choice) is to collect all the Pokémon Badges and conquer the Pokémon League. ";
        Utils::waitForEnter();
        std::cout << Utils::getPrintName(p.name) << "Wait... that sounds a lot like every other Pokémon game out there.";
        Utils::waitForEnter();
        std::cout << Utils::getPrintName(name)   << "Shhh! Don't break the fourth wall " << p.name  << "! This is serious business.";
        Utils::waitForEnter();
        std::cout << Utils::getPrintName(name)   << "To achieve this, you’ll need to battle wild Pokémon, challenge gym leaders, and of course, keep your Pokémon healthy at the PokeCenter.";
        Utils::waitForEnter();
        std::cout << Utils::getPrintName(name)   << "Along the way, you'll capture new Pokémon to strengthen your team. Just remember—there’s a limit to how many Pokémon you can carry, so choose wisely!";
        Utils::waitForEnter();
        std::cout << Utils::getPrintName(p.name) << "Sounds like a walk in the park... right?";
        Utils::waitForEnter();
        std::cout << Utils::getPrintName(name)   << "Hah! That’s what they all say! But beware, young Trainer, the path to victory is fraught with challenges. And if you lose a battle... well, let’s just say you'll be starting from square one.";
        Utils::waitForEnter();
        std::cout << Utils::getPrintName(name)   << "So, what do you say? Are you ready to become the next Pokémon Champion?";
        Utils::waitForEnter();
        ...
        Utils::waitForEnter();
        std::cout << Utils::getPrintName(name)   << "But first... let's just pretend I didn't forget to set up the actual game loop... Ahem, onwards!";
        Utils::waitForEnter();
    }
};
